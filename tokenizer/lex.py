...
class Evaluator:
    def __init__(self):
        self.code = None
        self.func_declarations: dict[str, list[int, int]] = {}

    # TODO: replace with LUT
    def back_prop(self, var_name, line_num, past_calls):
        print("=====back prop=====")
        print('looking for:', var_name, 'line_num:', line_num)
        line_num -= 1
        ex: Exp = None
        truncated_code = self.code[:line_num]
        ...
        for l in reversed(truncated_code):  # searching from line_num to top of page
            ...
            ex: Exp = self.eval(l, line_num, past_calls, just_kind=False)
            if ex is None:
                continue
            if ex.type == 'NUMBER':
                break
            if ex.type == 'ASSIGN' and ex.value == var_name:
                # if ex.type == 'ASSIGN':
                #     ex = self.back_prop(min_file_arr.index(l), ex.arg)
                break
            if ex.type == 'FUNC_DEC':
                for i in ex.arg:
                    if i.arg == var_name:
    ...
    def eval(self, expression: str, line_num: int, past_calls: set, just_kind: bool = False, suppress=False) -> Exp:
        ...
        for tokens in token_specification:
            ...
            if m := re.search(regex, expression):
                ...
                match kind:
                    case "FUNC_DEC":
                        ...
                        for i in m.group(2).split(','):
                            arr.append(self.eval(i, line_num, past_calls))
                        arg = arr
                    case "FUNC":
                        arg = m.group(2)
                        if not just_kind:  # and arg:
                            ...
                            if func_name in past_calls:  # protect against cyclic calls
                                ...
                                kind = "recursive_call"
                            elif func_name in self.func_declarations:
                                value = (m[1], m[2].split(','))
                                past_calls.add(func_name)
                                print("found:", expression)
                                start, end = self.func_declarations[func_name]
                                # ==
                                print('code', self.code[start:end + 1])
                                # kind = 'CLOSURE'
                                arg = self.code[start + 1:end]  # TODO: contains \n and empty lines
                                # ==
                                ...
                                if line_num < start or line_num > end:  # TODO: move up
                                    arr.append(self.eval(self.code[start], start + 1, past_calls))
                                else:
                                ...
                                past_calls.add(func_name)
                                arg = self.eval(arg, line_num, past_calls)
                    case 'CLOSURE':
                        # arg = m.group(2)
                        curl_count = 0
                        if not just_kind:
                            arr = []
                            i = line_num
                            while self.code[i] != '}' or curl_count > 0:
                                arr.append(self.code[i])
                                if self.code[i] == '{':
                                    curl_count += 1
                                elif self.code[i] == '}':
                                    curl_count -= 1
                    ...
                    case 'VAR':
                        if value in keywords:
                            break
                        t = self.back_prop(m[1], line_num, past_calls)
                        return t
                    case 'ASSIGN':
                        arg = m[2]
                        if not just_kind:
                    ...
                    case 'OP':
                        ...
                        arg1, arg2 = m[1], m[3]
                        if not (m[1] == '+' or m[1] == '-'):
                            arg1 = self.eval(m[1], line_num, past_calls)
                        if not (m[3] == '+' or m[3] == '-'):
                            arg2 = self.eval(m[3], line_num, past_calls)
                        arg = [arg1, arg2]
                    # case 'NEWLINE':
                    #     line_num -= 1
                    #     line_start = m.end()
                ...
                if not suppress:
                    print(t)
                if m.end() - m.start() != len(
                        expression):  # splitting the expression if only a part of it is understood
                    arg = [t]
    ...
    def preproc(self, file_name):
        ...
        self.code = re.split('([;{}])', min_file)
        self.code = [i for i in self.code if not (i == ';' or i == '')]  # remove semicolons
        # ==================== first scan ====================
        curr_func = ""
        curly_num = 0
        for line_num, line in enumerate(self.code):
            ...
            if exp and curly_num == 0:
                self.func_declarations[exp[1]] = [line_num + 1, None]
                curr_func = exp[1]
                ...
                curly_num -= 1
                if curly_num == 0:
                    self.func_declarations[curr_func][1] = line_num  # +1
        print("func declarations:", self.func_declarations)
        # ========================================
        ...
        offset = self.func_declarations['main'][0] - 1
        truncated_code = self.code[offset:]
        print("code", truncated_code)


if __name__ == '__main__':
    Evaluator().preproc("test.c")
