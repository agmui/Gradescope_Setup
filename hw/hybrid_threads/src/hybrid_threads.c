...
void create_new_parameterized_thread(void (*fun_ptr)(void*), void* parameter) {
   ...
   int thread_loc;
   // printf("Finding space for child thread\n");
    pthread_mutex_lock(&mutex);
   for(thread_loc = 0; thread_state[thread_loc]!=INVALID;thread_loc++){
      // printf("Checking %i for child thread: %d\n", thread_loc, child_done[thread_loc]);
      if (thread_loc == MAX_THREADS){
         exit(-1);
      }
   }

   printf("Creating child thread %i\n", thread_loc);
   thread_state[thread_loc] = CREATING;
    pthread_mutex_unlock(&mutex);
    getcontext(&threads[thread_loc]);
    threads[thread_loc].uc_link = 0;
    threads[thread_loc].uc_stack.ss_sp = malloc(THREAD_STACK_SIZE);
    threads[thread_loc].uc_stack.ss_size = THREAD_STACK_SIZE;
    threads[thread_loc].uc_stack.ss_flags = 0;
    makecontext(&threads[thread_loc], &thread_handler, 2, cast_ptr, parameter);
    thread_state[thread_loc] = PAUSED;
}
...
/*
   ...
   */
void* schedule_threads(void* voided) {
    bool allDone = true;
    for(int i=0; i < MAX_THREADS; i++){
        allDone &= !thread_state[i];
    }
    while(!allDone){
        allDone = true;
        for(int i=0; i < MAX_THREADS; i++){
            ...
            if(thread_state[i]==PAUSED){  
                threadNumber = i;
                printf("Switching to child thread %i %d\n", threadNumber, thread_state[threadNumber]);
                thread_state[threadNumber]=RUNNING;
                pthread_mutex_unlock(&mutex);
                swapcontext(&parent, &threads[threadNumber]);
                
                if(thread_state[threadNumber]==FINISHED){
                    free(threads[threadNumber].uc_stack.ss_sp);
                    thread_state[threadNumber]=INVALID;
                }else{
                    thread_state[threadNumber]=PAUSED;
                }
            }else{
                pthread_mutex_unlock(&mutex);
            }
            allDone &= (thread_state[i]==INVALID);
        }
   }
   return NULL;
}
...
*/
void yield() {
   printf("Switching to parent from child thread %i %d\n", threadNumber, thread_state[threadNumber]);
   swapcontext(&threads[threadNumber], &parent);
}
...
*/
void finish_thread() {
   printf("Child thread %i exiting\n", threadNumber);
   thread_state[threadNumber]= FINISHED;
   yield();
}

void thread_handler(void(*cast_ptr)(), void* parameter){
   cast_ptr(parameter);
   finish_thread();
}